<?php
if ( !class_exists( 'WPSBC_PluginUpdateChecker' ) ):
...
* @access public
*/
class WPSBC_PluginUpdateChecker {
	...
	function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			$pluginInfo = WPSBC_PluginInfo::fromJson($result['body']);
		}
	...
	function requestUpdate(){
		...
		return WPSBC_PluginUpdate::fromPluginInfo($pluginInfo);
	}
...
if ( !class_exists('WPSBC_PluginInfo') ):
...
if ( !class_exists('WPSBC_PluginUpdate') ):
...
class WPSBC_PluginUpdate {
	...
	public static function fromJson($json){
		...
		$pluginInfo = WPSBC_PluginInfo::fromJson($json);
		if ( $pluginInfo != null ) {
			return WPSBC_PluginUpdate::fromPluginInfo($pluginInfo);
		} else {
			return null;
